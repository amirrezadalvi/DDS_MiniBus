...
$env:CMAKE_PREFIX_PATH = Join-Path $qtPath "lib\cmake"
$env:PATH = (Join-Path $qtPath "bin") + ";" + $mingwPath + ";" + $env:PATH
Write-Host "Using Qt: $qtPath"
Write-Host "Using MinGW: $mingwPath"
if (Test-Path build) { Remove-Item build -Recurse -Force }
cmake -S . -B build -G "MinGW Makefiles" -DCMAKE_PREFIX_PATH="$env:CMAKE_PREFIX_PATH"
cmake --build build -j